...
from app.graph.signal_workflow import signal_graph

members= ["research_team", "case_management_team" , "signal_team"]
...
system_prompt = (
        ...
        "1.  **KEYWORD SCAN (HIGHEST PRIORITY):** Scan the user's entire query for specific keywords. "
        "The first match in the following priority order determines the destination. Check for Signal Team keywords *before* Case Management keywords.\n"
        "    - **Priority 1 (Signal Team):** If the query contains **'signal', 'signals', 'rule', 'rules', 'violation'**, you MUST route to the `signal_team`.\n"
        "    - **Priority 2 (Case Management Team):** If no Signal Team keywords are found, but the query contains **'incident', 'transaction', 'focal party', 'INC-', 'TRN-', 'FP-'**, you MUST route to the `case_management_team`.\n\n"
        "2.  **GENERAL RESEARCH (LOWER PRIORITY):** If and only if NONE of the high-priority keywords from "
        "Rule #1 are found, and the query is a general question (e.g., 'what is...', 'tell me about...'), "
        ...
        "## YOUR RESPONSE:\n"
        "Your final response must be a single word: either a team name or `FINISH`."
)
...
def call_signal_team(state: State) -> Command[Literal["supervisor"]]:
    response = signal_graph.invoke({"messages": state["messages"][-1]})
    return Command(
        update={
            "messages": [
                HumanMessage(
                    content=response["messages"][-1].content, name="signal_team"
                )
            ]
        },
        goto="supervisor",
